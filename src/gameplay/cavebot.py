...
def shouldAskForCavebotTasks(context: Context) -> bool:
    ...
    if currentTask is None:
        return True
    return (currentTask.name not in ['dropFlasks', 'lootCorpse', 'moveDown', 'moveUp', 'refillChecker', 'singleWalk', 'refillChecker', 'useRopeWaypoint', 'useShovelWaypoint', 'useLadderWaypoint', 'goDownHoleWaypoint', 'toggleTargetingWaypoint'])
